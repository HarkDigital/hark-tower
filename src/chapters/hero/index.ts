 ...
 *
 *   0.00–0.10  THE VISION. Street level outside the hoarding. After the loader
 ...
 *              the dawn; then it hands over to the world's ghost.
 *              Intro plate: eyebrow, manifesto, scroll hint.
 ...
 *   0.56–0.93  PAYOFF. Out to the street corner, ultra-wide and low: the whole
 *              vision standing over the first steel. "Make the internet listen."
...
export default function create(): Chapter {
  ...
  const computeShot = (local: number, frame: Frame, rev: number) => {
    ...
    if (rev < 1) {
      // the reveal: the camera starts on the hoarding and tilts up with the pen
      const k = ease.inOutCubic(clamp(rev))
      shot.ty = lerp(portrait ? 12 : 7, shot.ty, k)
      shot.fov = lerp(shot.fov - 8, shot.fov, k)
      // portrait starts square-on to the project board, then swings onto the tower
      if (portrait) shot.side = lerp(12.6, shot.side, k)
    }
    place(shot, pos, tgt)
    fov = shot.fov
  }
  ...
  return {
    ...
    async init(ctx) {
      ...
      // ---- DOM: intro spec plate
      intro = el('div', 'th-intro hud-panel', undefined, ctx.stage)
      el('p', 'hud-eyebrow', MICROCOPY.signalEyebrow, intro)
      el('p', 'hud-body th-manifesto', BRAND.manifesto, intro)
      const hint = el('p', 'hud-label th-hint', undefined, intro)
    ...
    update(local, frame, ctx) {
      ...
      wp.glazed = 0
      wp.fitted = 0
      // while the pen is drafting, the world's own ghost waits; then it takes over
      wp.ghost = draw >= 0.97 ? 1 : 0
      wp.focus.set(0, 0, 12)
      wp.fog = lerp(1.15, 1, smoothstep(0.1, 0.3, local))
      const actualBuilt = ctx.world.tower.frontier / FLOOR_H

      // the drawing: pen for the reveal, then fade to the world's ghost
      const vf = reduced ? 0 : 1 - smoothstep(1.0, 1.3, rev)
      setDraw(vision.ghost, draw, vf)
      setDraw(vision.crown, draw, vf)
      // the dimension strings stay while the vision holds the screen
      setDraw(vision.dims, reduced ? 1 : draw, 1 - smoothstep(0.07, 0.13, local))
      ...
      site.lampMat.color.set('#ffd7a0').multiplyScalar(lerp(5, 3, smoothstep(0.6, 1, local)))
      ...
      // ---------------- DOM
      reveal(intro, (1 - smoothstep(0.075, 0.12, local)) * (reduced ? 1 : smoothstep(0.55, 0.95, draw)))
      ...
      setRise(title, local > 0.645 && local < 0.95)
